...
export function* fetchDonations(action: any) {
  try {
    const token: IAccessToken = yield select(
    ...
    const filter: IDonationFilter = yield select(
      (state: AppState) => state.donations.filter,
    );
    ...
    yield put(
      fetchDonationsAction.failed({ params: action.payload, error: ex }),
    );
  }
}
...
export function* deleteDonation(action: any) {
  try {
    ...
    yield put(
      deleteDonationAction.failed({ params: action.payload, error: ex }),
    );
  }
}
