...
} from './list/donations-list.actions';
import {
  fetchDonationAction,
  CLEAR_CURRENT_DONATION,
  fetchHistogramAction,
} from './donation/donation.actions';
...
const defaultState: DonationsState = {
  ...
  filter: {
    ...
    paymentMethodIDs: [2, 3, 4, 5, 6],
  },
};
export const donationsReducer = (
  state = defaultState,
  action: any,
): DonationsState => {
  /**
  ...
  if (isType(action, fetchDonationsAction.started)) {
    return { ...state, loading: true };
  }
  if (isType(action, fetchDonationsAction.failed)) {
  ...
  if (isType(action, fetchDonationAction.failed)) {
  ...
  if (isType(action, fetchHistogramAction.failed)) {
