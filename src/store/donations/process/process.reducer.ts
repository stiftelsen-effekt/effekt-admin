  ...
  if (isType(action, uploadReportAction.failed)) {
    ...
    if (action.payload.result.invalid > 0) {
      ...
      return {
        ...
        invalidTransactions: action.payload.result.invalidTransactions.map(
          (invalid) => {
            return {
    ...
    const transactions = state.invalidTransactions;
    transactions.pop();
    ...
    const externalRef = action.payload.params.donation.paymentExternalRef;

    const transactions = state.invalidTransactions.filter(
