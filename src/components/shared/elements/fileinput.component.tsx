...
export const EffektFileInput: React.FunctionComponent<IProps> = (
  props: IProps,
) => {
  ...
  if (state.filename) {
    label = (
      <FileInputLabel htmlFor={props.id}>{state.filename}</FileInputLabel>
    );
  } else {
    label = (
  ...
  return (
    <div>
      <HiddenFileInput {...props} type="file" onChange={selectedFileChanged} />
      {label}
    </div>
  );
};
