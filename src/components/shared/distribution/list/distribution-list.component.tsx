...
} from '../../../../store/distributions/list/distribution-list.actions';

export const DistributionsList: React.FunctionComponent = () => {
  const data = useSelector(
    (state: AppState) => state.distributions.searchResult,
  );
  ...
  const columnDefinitions = [
    ...
    },
    {
      ...
      accessor: (res: any) => {
        if (res.sum)
          return res.sum.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ');
        return '';
      },
    },
    {
  ...
  if (distribution !== null)
    return <Redirect to={`/distributions/${distribution}`} />;
  return (
