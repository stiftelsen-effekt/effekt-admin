...
export const DistributionInput: React.FC = () => {
  ...
  const distributionInput: IDistributionShare[] = useSelector(
  ...
  return (
    <Wrapper>
        ...
        {organizations.map((org) => (
          <div key={org.id}>
            <TextInput
              ...
              placeholder="0"
              value={
                distributionInput.filter((dist) => dist.organizationId === org.id).length === 1
                  ? distributionInput
                      .filter((dist) => dist.organizationId === org.id)[0]
                      .share.toString()
                  : ''
              }
      ...
      {sumPercentage === 100 ? null : (
        <RedFont>{`You have distributed ${sumPercentage} of 100 percent`}</RedFont>
      )}
    </Wrapper>
  );
};
