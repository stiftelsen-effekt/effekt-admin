...
import { DonorSelectionWrapper, SelectionWrapper, DonorSelectionInput, DonorProperty } from "./Donor.style";
...
export const KIDDonorComponent: React.FunctionComponent<IProps> = (props) => {
    ...
    if (donor !== undefined) {
        if (donor.id !== parseInt(donorId)) 
            setDonorId(donor.id.toString())
    ...
    return (
        <DonorSelectionWrapper>
            <div style={{ display: 'flex', flexGrow: 1, alignItems: 'center' }}>
                <DonorSelectionInput>
                    <strong>Donor</strong>
                    <EffektInput value={donorId} onChange={handleChange} style={{ width: '70px' }} placeholder="DonorID"></EffektInput>
                </DonorSelectionInput>
                {selectedDonor}
            </div>
