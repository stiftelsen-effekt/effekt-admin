...
export const DonorKeyInfo: React.FunctionComponent<{ donor: IDonor }> = ({ donor }) => {
  ...
  useEffect(() => {
    if (updateError?.message)
      toastError("Donor not modified", updateError.message)
    ...
    <DonorKeyInfoWrapper>
      <table className={pendingUpdates ? "updating" : undefined} width="100%" cellSpacing={0}>
        <tbody>
          ...
          <DonorKeyInfoField donor={donor} field="email" label="Email" validate={Validator.isEmail} editable={false} />
          <DonorKeyInfoField donor={donor} field="ssn" label="SSN/OrgNr." validate={validateSSNOrOrgnr} />
          <DonorKeyInfoField donor={donor} field="newsletter" type={FieldType.BooleanInput} label="Newsletter" />
          <DonorKeyInfoField donor={donor} field="trash" type={FieldType.BooleanInput} label="Trash" />
          <DonorKeyInfoField donor={donor} field="registered" type={FieldType.DateTimeInput} label="Registered" validate={(val: DateTime) => val.isValid} editable={false} />
        </tbody>
      </table>
