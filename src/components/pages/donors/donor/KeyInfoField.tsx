...
export const DonorKeyInfoField: React.FunctionComponent<{
  label: string;
  field: string;
  type?: FieldType;
  donor: IDonor;
  validate?: Function;
  editable?: boolean;
}> = ({ label, field, type, donor, validate, editable }) => {
  ...
  const toTextValue = (val: any) => {
    if (type === FieldType.DateTimeInput) return val.isValid ? longDateTime(val as DateTime) : "";
    if (type === FieldType.BooleanInput) return val ? <Check size={20} /> : <X size={20} />;
    return val;
  };
  ...
  const fromFieldValue = (val: string | boolean) => {
    ...
    if (type === FieldType.BooleanInput) return Boolean(val);
    return val;
  };
      ...
      <>
        <tr>
          ...
          <td>
            {editing ? (
              <input
                className={validValue ? "valid" : "invalid"}
                type={String(type)}
                checked={Boolean(newValue)}
                value={toFieldValue(newValue)}
                onChange={(e) => {
                  const val = fromFieldValue(
                    type === FieldType.BooleanInput
                      ? e.currentTarget.checked
                      : e.currentTarget.value,
                  );
                  setNewValue(val);
                  if (validate) setValidValue(validate(val));
                }}
              />
            ) : (
    ...
    return (
      <>
        <tr>
          <td>{label}</td>
          <td>
            <span className="valid">{toTextValue(newValue)}</span>
          </td>
          <td />
          <td />
        </tr>
